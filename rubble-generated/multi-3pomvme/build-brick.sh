...
case "${1:-}" in
  cleanup)
    ...
    exit 0
    ;;
  build|release) job_kind="$1"; shift ;;
  --run-job) ;;
  *) job_kind=build ;;
esac
...
if [[ "${job_kind}" == build ]]; then
  [[ "$#" -ge 1 ]] || fail "build requires a Brick file"
  for payload in "$@"; do payloads+=("$(absolute_payload "${payload}")"); done
fi
...
if [[ "${job_kind}" == build ]]; then
  ...
  unset RUBBLE_BRICK_FILE RUBBLE_BRICK_ID RUBBLE_JOB_BUILD_ALLOWED
fi
export RUBBLE_CI_JOB_KIND="${job_kind}"
...
if [[ "${job_kind}" == release ]]; then
  release_relative='release-plan.sh'
  exec "${rubble}" script --inherit-env --language bash "${script_dir}/${release_relative}"
fi
